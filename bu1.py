...
def all_data(id):
    all_data={}
    all_data['dayinfo']=burs.get_dayinfo(id)
    all_data['dayprice']=burs.get_dayprice(id)
    all_data['clienttype_history']=burs.get_clienttype_history(id)
    return all_data
def saf_pd(id):
    day=burs.get_dayinfo(int(id))
    saf_np=np.empty((0,6))
    for i in range(1,4):
...
def general_dayinfo_pd(id):
    general=burs.get_day_general_info(int(id))
...
def dayprice_plot(id):
    a=dayprice_pd(int(id))
    fig = plt.figure()
    ax = fig.add_subplot(111)
    pltfinance.candlestick2_ochl(ax, a.open_price, a.close_price, a.high_price, a.low_price, width=1, colorup='w', colordown='k', alpha=1)
    return fig
...
#    dayprice_np=np.concatenate((dayprice_np,np.array([list(i.values())])))
#dayprice_np[:,1:]=dayprice_np[:,1:].astype(int)
#dayprice_pd=pd.DataFrame(dayprice_np,columns=list(i.keys()))
#now=pd.datetime.now()
#for i in range(len(dayprice_pd.time)):
#    s=dayprice_pd.time[i].split(':')
#    dayprice_pd.time[i]=pd.datetime(now.year,now.month,now.day,int(s[0]),int(s[1]))
